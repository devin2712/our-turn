...
const convertHTML = (user, statuses) => {
  ...
  const locationPreferencePhrase = (user.locations && JSON.parse(user.locations).length > 0) ? "the following locations: " + JSON.parse(user.locations).join(', ') : "all available locations.";
